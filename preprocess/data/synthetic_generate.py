...
if __name__ == '__main__':
    ...
    theta_samples = np.linspace(0.2*np.pi/2.0, 0.9*np.pi/2.0, num=20)
    ...
    for i, c in enumerate(cam):      
        ...
        cv2.imwrite('%s/video_frames/%s.JPG' % (args.output_dir, frame_name), img)

        np.save('%s/video_frames_extrinsics/%s.npy' % (args.output_dir, frame_name), c)
    ...
    for i, c in enumerate(cam):      
        ...
        img = film.bitmap(raw=True).convert(Bitmap.PixelFormat.RGB, Struct.Type.UInt8, srgb_gamma=True)
        img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
        cv2.imwrite('%s/video_frames_test/%s.JPG' % (args.output_dir, frame_name), img)

        np.save('%s/video_frames_test_extrinsics/%s.npy' % (args.output_dir, frame_name), c)
