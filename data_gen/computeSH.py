...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', type=str, default='pinecone_dr.xml')
    ...
    im = cv2.imread(args.env, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    im = cv2.resize(im, (args.img_width, args.img_height))
    im = im[:, :, ::-1]
    coef = computeSHFromImage(im)
    print(coef)
    np.save('%s/train/envSH.npy' % args.output_dir, coef)
    np.save('%s/test/envSH.npy' % args.output_dir, coef)
