...
if __name__ == '__main__':
    ...
    for i, img_name in enumerate(img_list):
        process(args, i, img_name, frames_dir, args.output_dir+'/train/')

    # mask generation
    # frames_dir = 'video_frames'
    # model_name = 'u2net'
    # images_dir = f'{args.data_dir}/{frames_dir}/'
    # prediction_dir = f'{args.output_dir}/masks/'
    # model_dir = os.path.join(os.getcwd(),f'{args.model_path}/{model_name}/{model_name}.pth')
    # segment_object(model_name,images_dir,prediction_dir,model_dir)    
    
    #--Test--
    frames_dir = 'video_frames_test'
    img_list = sorted(os.listdir('%s/%s/' % (args.data_dir, frames_dir)))
    for i, img_name in enumerate(img_list):
        process(args, i, img_name, frames_dir, args.output_dir+'/test/')
